...
public class UserModel extends BaseModel<UserBean> {
	...
	public void updatePhoto(long id, String photo) throws ApplicationException {
		...
		try {
			...
			try {
				...
				throw new ApplicationException(
						"Exception : updatePhoto rollback exception " + ex.getMessage());
			}
	 ...
	 * @return authenticated UserBean, or null if authentication fails
	 ...
	 * @param login user login
	 * @throws ApplicationException if an application error occurs
	 * @throws RecordNotFoundException if user is not found
	 */
	 ...
	 * @throws RecordNotFoundException if user is not found or old password is incorrect
	...
	public long registerUser(UserBean bean)
		...
		try {
			...
			msg.setSubject("Registration is successful for ORS Project  Durgesh Lohiya");
			msg.setMessage(message);
			msg.setMessageType(EmailMessage.HTML_MSG);
